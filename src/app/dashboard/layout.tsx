...
export default function DashboardLayout({
  ...
  useEffect(() => {
    // Check if both Supabase and NextAuth are done loading
    const isLoading = loading || nextAuthStatus === "loading";

    // User is authenticated if they have either a Supabase user or NextAuth session
    const isAuthenticated = user || nextAuthSession;

    if (!isLoading && !isAuthenticated) {
      router.push("/auth/login");
    }
  }, [user, loading, nextAuthSession, nextAuthStatus, router]);

  useEffect(() => {
    async function fetchAvatar() {
      // For Supabase users, get avatar from storage
      ...
      // For NextAuth users, use the image directly
      else if (nextAuthSession?.user?.image) {
        setAvatarUrl(nextAuthSession.user.image);
      } else {
        setAvatarUrl(null);
      }
    }
  ...
  // Get user info from either Supabase or NextAuth
    ...
    <div className="min-h-screen flex bg-gradient-to-br from-blue-50 via-fuchsia-50 to-emerald-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      ...
      <aside className="hidden md:flex flex-col w-64 h-screen sticky top-0 bg-white/70 dark:bg-gray-900/70 shadow-2xl border-r border-white/20 z-20 backdrop-blur-lg">
        <div className="flex flex-col items-center justify-center h-28 gap-2 select-none">
          ...
          <div className="text-sm font-semibold text-gray-800 dark:text-gray-100 truncate max-w-[90%]">
            {userEmail}
          </div>
        </div>
        <nav className="flex-1 flex flex-col gap-2 p-4">
          {tabs.map((tab) => (
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-lg font-semibold transition-all duration-200 group
                ${
                  pathname.startsWith(tab.href)
                    ? "bg-gradient-to-r from-blue-600 via-fuchsia-500 to-emerald-500 text-white shadow-lg scale-105"
            ...
            >
              ...
              {tab.name === "Profile" && (
                <svg
                  className="w-4 h-4 ml-auto text-gray-400 group-hover:text-white"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  viewBox="0 0 24 24"
              ...
              {pathname.startsWith(tab.href) && (
                <span className="ml-auto w-2 h-2 rounded-full bg-white/80 animate-pulse" />
              )}
            </Link>
          ))}
        </nav>
        <div className="mt-auto p-4 flex flex-col gap-2">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-fuchsia-500 to-blue-600 text-white font-semibold shadow hover:scale-105 transition-all"
          >
            <svg
              className="w-5 h-5"
              fill="none"
      ...
      >
        <svg
          className="w-7 h-7 text-blue-600 dark:text-fuchsia-400"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          viewBox="0 0 24 24"
      ...
      >
        <div
          className={`absolute left-0 top-0 h-full w-64 bg-white/90 dark:bg-gray-900/90 shadow-2xl border-r border-white/20 transform transition-transform duration-300 backdrop-blur-lg ${
