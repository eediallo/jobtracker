...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (user && user.user_metadata) {
      if (user.user_metadata.cv_url) {
        setCv({
          name: user.user_metadata.cv_name || "Curriculum Vitae",
          url: user.user_metadata.cv_url,
        });
      }
      if (user.user_metadata.cl_url) {
        setCoverLetter({
          name: user.user_metadata.cl_name || "Cover Letter",
          url: user.user_metadata.cl_url,
        });
      }
    }
  }, [user]);
  ...
  async function handleFileUpload(
    ...
    if (updateUserError) {
      ...
      toast.success(
        `${type === "cv" ? "CV" : "Cover Letter"} uploaded successfully!`
      );
    }
  }
  ...
  // For NextAuth users, show a different message since profile management is not available
  if (!user && nextAuthSession) {
    return (
      <main className="max-w-3xl mx-auto p-4">
        <h1 className="text-3xl font-bold mb-8 tracking-tight text-center md:text-left w-full block">
          Profile
        </h1>
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-6">
          <div className="text-center">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-500 via-fuchsia-400 to-emerald-400 flex items-center justify-center text-white text-2xl font-bold shadow-lg mx-auto mb-4">
              {nextAuthSession.user?.image ? (
                ...
                nextAuthSession.user?.email?.charAt(0).toUpperCase() || "?"
              )}
            </div>
            <h2 className="text-xl font-semibold mb-2">
              {nextAuthSession.user?.name || nextAuthSession.user?.email}
            </h2>
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              {nextAuthSession.user?.email}
            </p>
            <p className="text-gray-500 dark:text-gray-400 text-sm">
              You&apos;re signed in with Google. Profile management features are
              currently only available for email/password accounts.
            </p>
          </div>
        </div>
      </main>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-6 flex flex-col md:flex-row gap-10">
        ...
        <div className="flex-1 flex flex-col gap-10">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                {cv ? (
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400 p-2 bg-white dark:bg-gray-700 rounded-lg border border-dashed">
                    No CV uploaded.
                  </div>
                )}
                <button
          ...
          <form
            ...
            className={`grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 items-end transition-all duration-300 ${
              edit ? "opacity-100" : "pointer-events-none opacity-60"
            }`}
            style={{
          ...
          >
            <div className="col-span-2 font-semibold text-gray-700 dark:text-gray-200 mb-2">
              Change Password
            </div>
            <div className="relative">
              <input
                ...
                aria-invalid={!!error}
              />
              <label className="absolute left-4 top-2 text-gray-500 text-sm transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:text-base peer-focus:top-2 peer-focus:text-sm bg-white dark:bg-gray-900 px-1 pointer-events-none">
            ...
            {error && (
              <div className="col-span-2 flex items-center gap-2 text-red-500 text-sm mt-1">
                <svg
                  className="w-4 h-4"
                  fill="none"
                  stroke="currentColor"
            ...
            {message && (
              <div className="col-span-2 text-green-600 text-sm mt-1">
                {message}
              </div>
            )}
          </form>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between group">
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full transition-colors duration-200 ${
                  notif ? "bg-[#007bff]" : "bg-gray-300"
                } relative focus:outline-none focus:ring-2 focus:ring-blue-400`}
            ...
            <div className="flex items-center justify-between group">
              ...
              <button
                type="button"
                className={`w-12 h-6 rounded-full transition-colors duration-200 ${
                  privacy ? "bg-[#28a745]" : "bg-gray-300"
                } relative focus:outline-none focus:ring-2 focus:ring-green-400`}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl p-8 max-w-sm w-full flex flex-col items-center gap-6 border-2 border-red-200">
            ...
            <div className="text-gray-700 dark:text-gray-200 text-center mb-4">
